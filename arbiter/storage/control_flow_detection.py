...
class Marker(object):
    def __init__(self, marker: N):
        """
        Base class for storing results from Arbiter control flow analysis
        """
        self._marker = marker
...
class SinkMarker(Marker):
    ...
    def __str__(self) -> str:
        return f"{str(self.marker)} in {self.function.name} @ {hex(self.function.addr)} <block {hex(self.block.addr)}>"
